...
export const totpRegister = async (_2fa_code: string): Promise<any> => {
  try {
    const response = await axiosInstance.post(
      "/user/me/totp_register?_2fa_code=" + _2fa_code
    );
    return response.data;
  } catch (error) {
...
export const totpRemove = async (
  ...
  try {
    ...
    const response = await axiosInstance.post("/user/me/totp_remove", requestBody);
    return response.data;
  } catch (error) {
    console.error("Error during totpRemove call:", error);
    throw error;
  }
};
