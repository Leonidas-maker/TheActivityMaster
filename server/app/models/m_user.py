...
class User(Base):
    ...
    username: Mapped[Annotated[str, mapped_column(String(50), nullable=False, unique=True)]]
    ...
    created_at: Mapped[
        Annotated[
            datetime,
            mapped_column(DateTime(timezone=True), nullable=False, default=lambda: datetime.now(DEFAULT_TIMEZONE)),
        ]
    ]
    updated_at: Mapped[
        Annotated[
...
class User2FA(Base):
    ...
    key_handle: Mapped[
        Annotated[str, mapped_column(String(255), nullable=True)]
    ]  # Secret key for TOTP | Key handle for U2F
    counter: Mapped[Annotated[int, mapped_column(Integer, nullable=True)]]  # Last used counter for TOTP or U2F
    fails: Mapped[
        Annotated[int, mapped_column(Integer, nullable=False, default=-1)]
...
class UserPasskey(Base):
    ...
    id: Mapped[Annotated[uuid.UUID, mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)]]
    user_id: Mapped[Annotated[uuid.UUID, mapped_column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=False)]]
    credential_id: Mapped[
        Annotated[str, mapped_column(String(255), nullable=False, unique=True)]
    ]  # Generated by WebAuthn
    public_key: Mapped[Annotated[str, mapped_column(Text(1000), nullable=False)]]  # Public key generated by WebAuthn
    sign_count: Mapped[
        Annotated[int, mapped_column(Integer, nullable=False, default=0)]
