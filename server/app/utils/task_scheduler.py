...
#     def __init__(self, redis_url="redis://localhost:6379/0", max_block_time: int = 4, verbose: bool = False):
#         """
#         Initializes the TaskScheduler with Redis job store.

#         Args:
#             redis_url (str): Redis URL for storing jobs.
#             max_block_time (int): Maximum block time in seconds.
#             verbose (bool): Enables verbose logging if set to True.
#         """
#         self.jobstores = {
#             "default": RedisJobStore(jobs_key="apscheduler.jobs", run_times_key="apscheduler.run_times", host="localhost", port=6379, db=0)
#         }
...
#             try:
#                 db = SessionLocal()
#                 result = func(*wrapper_args, **kwargs, db=db)
#                 db.close()
#                 if result:
